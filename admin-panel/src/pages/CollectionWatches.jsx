...
const CollectionWatches = () => {
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      const [colRes, watchesRes] = await Promise.all([
        axios.get(`${config.API_BASE_URL}/collections/${id}`),
      ...
      (watchesRes.data || []).forEach((w) => {
        const coll = w.collectionRef?._id ? w.collectionRef._id : w.collectionRef;
        if (String(coll) === String(id)) initial.add(String(w._id));
      });
      setSelected(initial);
      setLoading(false);
    };
  ...
  const handleSave = async () => {
    ...
    for (const w of watches) {
      ...
      const coll = w.collectionRef?._id ? String(w.collectionRef._id) : (w.collectionRef ? String(w.collectionRef) : null);
      const currentlyHas = coll === String(id);
      if (has && !currentlyHas) {
        // set collectionRef to id
        changed.push({ id: w._id, data: { collectionRef: id } });
      } else if (!has && currentlyHas) {
        // remove collectionRef
        changed.push({ id: w._id, data: { collectionRef: null } });
      }
    }
    ...
    try {
      ...
      alert("Collection assignments updated");
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <h2 className="text-3xl font-serif font-bold text-slate-900">Manage watches for collection</h2>
          <p className="text-slate-500 text-sm">Assign existing watches to this collection without recreating them.</p>
        </div>
